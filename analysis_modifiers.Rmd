...
betweenness_centrality %>% as.data.frame() %>% rownames_to_column() %>% rename(c(cxn = "rowname", betweenness_centr = `.`)) %>% datatable() %>% formatSignif(columns = "betweenness_centr", digits = 3)
...
# empty dataframe
mydf <- data.frame(matrix(NA, nrow = 9, ncol = 9))
colnames(mydf) <- cxns
rownames(mydf) <- cxns
...
# get smaller n and average of both n's to compute
# two different relative frequencies
mydf_long$n_min_types <- ifelse(mydf_long$types_A < mydf_long$types_B, mydf_long$types_A, mydf_long$types_B)

mydf_long$n_types_average <- (mydf_long$types_A + mydf_long$types_B) / 2

# "relative" frequency
mydf_long$rel <- mydf_long$n / mydf_long$n_min_types
mydf_long$rel2 <- mydf_long$n / mydf_long$n_types_average


# heatmap with color displaying frequency relative to
...
mydf_long %>% filter(rel < 1) %>% ggplot(aes(x = cxn_A, y = cxn_B, fill = rel, label = n)) + geom_tile() + geom_text() + scale_fill_gradient(low = "yellow", high = "darkred") +
  ...
  guides(fill = "none") + theme_classic() + theme(axis.text.x = element_text(angle=45, hjust=.9))

# heatmap with color displaying frequency relative to
# mean number of types
mydf_long %>% filter(rel < 1) %>% ggplot(aes(x = cxn_A, y = cxn_B, fill = rel2, label = n)) + geom_tile() + geom_text() + scale_fill_gradient(low = "yellow", high = "darkred") +
  xlab("Construction") + ylab("Construction") + 
  guides(fill = "none") + theme_classic() + theme(axis.text.x = element_text(angle=45, hjust=.9))

# heatmap with color displaying absolute frequency
