...
# heatmap
mydf_long2 %>% filter(rel < 1) %>% ggplot(aes(x = x, y = cxn_B, fill = rel, label = shared)) + geom_tile() + geom_text() + scale_fill_gradient(low = "yellow", high = "darkred") +
  xlab("Construction") + ylab("Construction") + 
  guides(fill = "none") + theme_classic() + theme(axis.text.x = element_text(angle=45, hjust=.9))

# ggsave("heatmap001_all_types.png", width = 6, height = 6)

# or maybe try to plot the "base" collexemes on the y axis?
