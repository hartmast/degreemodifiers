...
top100_collexemes <- c(
col_fuenk %>% select(COLLEX) %>% head(100),
col_handvoll %>% select(COLLEX) %>% head(100),
col_hauch %>% select(COLLEX) %>% head(100),
col_idee %>% select(COLLEX) %>% head(100),
col_quaentchen %>% select(COLLEX) %>% head(100),
col_spur %>% select(COLLEX) %>% head(100),
col_tack %>% select(COLLEX) %>% head(100),
col_tick %>% select(COLLEX) %>% head(100),
col_zack %>% select(COLLEX) %>% head(100)
) %>% unique %>% unlist
...
# create long list of lemmas and cxns
lemmas_df <- rbind(
  mutate(select(filter(fuenk, Lemma %in% top100_collexemes), Lemma), cxn = "Fünkchen"),
  mutate(select(filter(hauch, Lemma %in% top100_collexemes), Lemma), cxn = "Hauch"),
  mutate(select(filter(idee, Lemma %in% top100_collexemes), Lemma), cxn = "Idee"),
  mutate(select(filter(quaentchen, Lemma %in% top100_collexemes), Lemma), cxn = "Quäntchen"),
  mutate(select(filter(spur, Lemma %in% top100_collexemes), Lemma), cxn = "Spur"),
  mutate(select(filter(tack_zack, Lemma %in% top100_collexemes), Lemma), cxn = "Tacken/Zacken"),
  mutate(select(filter(tick, Lemma %in% top100_collexemes), Lemma), cxn = "Tick")
)
